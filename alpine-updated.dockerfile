...
ARG DEF_VNC_RESOLUTION=1600x1200
ARG DEF_VNC_PASSWORD=mypassword
ARG DEF_VNC_PORT=5900
ARG DEF_NOVNC_WEBSOCKIFY_PORT=6080
ARG DEF_STARTING_WEBSITE_URL=https://news.bbc.co.uk
...
COPY browser_conf/policies.json /etc/firefox/policies/policies.json

# Set-up ublock origin for Firefox
RUN cd /tmp && wget https://github.com/gorhill/uBlock/releases/download/1.65.0/uBlock0_1.65.0.firefox.signed.xpi && \
    mkdir -p /usr/lib/firefox/distribution/extensions && \
    mv /tmp/uBlock0_1.65.0.firefox.signed.xpi /usr/lib/firefox/distribution/extensions/{c3cbe70e-9f1e-49ff-bc1b-49113c1a01ab}.xpi
